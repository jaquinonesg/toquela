...
class PaginatorJorge {
    ...
    public function getHtmlPaginator($isajax = false, $tamamo = 2, $format_url = "") {
        if ($this->process) {
            ...
            $html = '<div class="_paginator_jorge ' . $this->clase_alias . '" style="text-align: center;" total_elementos="' . $this->total_elementos . '" num_elementos_por_pagina="' . $this->num_elementos_por_pagina . '" pagina_seleccionada="' . $this->pagina_seleccionada . '" num_paginas="' . $this->num_paginas . '" inicio_limit="' . $this->inicio_limit . '" fin_limit="' . $this->fin_limit . '" isajax="' . ($this->isajax ? 'true' : 'false') . '">'
                    . '<ul class="' . $classtamano . '">'
                    . '<li id="_btn_back" style="cursor: pointer;"><a>&laquo;</a></li>';
            ...
            for ($p = $desde; $p <= $hasta; $p++) {
                if ($p == $this->pagina_seleccionada) {
                    $html .= '<li class="active _pag" pag="' . $p . '" style="cursor: pointer;"><a>' . $p . '</a></li>';
                } else {
                    if ($this->isajax) {
                        $html .= '<li class="_pag" pag="' . $p . '" style="cursor: pointer;"><a>' . $p . '</a></li>';
                    } else {
                        $html .= '<li class="_pag" pag="' . $p . '" style="cursor: pointer;"><a href="' . $format_url . $p . '">' . $p . '</a></li>';
                    }
                }
            }
            $html .= '<li id="_btn_next" style="cursor: pointer;"><a>&raquo;</a></li></ul>' . $htmlcontrols . '</div>';
            return $html;
        }
