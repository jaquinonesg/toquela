...
class ComplejosController extends Controller {
    ...
    public function index($render_iframe = NULL) {
        $this->_view->setTitle("Mis Canchas");
        $this->_view->setItems('only', array('listar_canchas.js'));
        
        if ($render_iframe == "loadiframe") {
            $total_complex = DAOFactory::getComplexDAO()->getComplexBuscador("", null, null, true, true);
        } else {
        ...
        if ($render_iframe == "loadiframe") {
            $complex = DAOFactory::getComplexDAO()->getComplexBuscador("", $pj->inicio_limit, $pj->fin_limit, false, true);
        } else {
            $complex = DAOFactory::getComplexDAO()->getComplexBuscador("", $pj->inicio_limit, $pj->fin_limit);
        }
        $htmlpaginator = $pj->getHtmlPaginator(true, 2, SITE . "/complejos/");
    ...
    public function search_canchas($render_iframe = NULL) {
        $data["html"] = "";
        ...
        if ($render_iframe == "loadiframe") {
            $this->_view->assign('isiframe', true);
            $this->_view->assign("template", "listar_canchas_iframe");
        } else {
        ...
        if ($render_iframe == "loadiframe") {
            $total_complex = DAOFactory::getComplexDAO()->getComplexBuscador($strcancha, null, null, true, true);
        } else {
        ...
        if ($render_iframe == "loadiframe") {
            $complex = DAOFactory::getComplexDAO()->getComplexBuscador($strcancha, $pj->inicio_limit, $pj->fin_limit, false, true);
        } else {
    ...
    public function publico($complejo = null, $isiframe = null) {
    ...
    public function setCalificacion() {
        if (isset($_POST['num_expe']) && isset($_POST['num_inst'])) {
            ...
            //http://localhost/toquela/complejos/disponibilidad?complejo=1
//            header ("Location: http://www.cristalab.com");
            header("Location: " . SITE . "/complejos/disponibilidad?complejo=" . $_POST['complex']);
        }
    }
