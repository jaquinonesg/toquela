 ...
 */
 class RoleMySqlDAO extends ModelDAO implements RoleDAO{
    ...
    public function insert($role){
        $this->set($role->name);
        $this->set($role->descripcion);
        $this->set($role->privilegios);
        
        $sql = "INSERT INTO db_toquela.role ( name , descripcion , privilegios ) 
        VALUES ('$role->name','$role->descripcion','$role->privilegios')";
    ...
    public function update($role){
        ...
        name = if( strcmp('$role->name'  , '' ) = 1  , '$role->name', name ),
        descripcion = if( strcmp('$role->descripcion'  , '' ) = 1  , '$role->descripcion', descripcion ),
        privilegios = if( strcmp('$role->privilegios'  , '' ) = 1  , '$role->privilegios', privilegios ) WHERE  cod_role =  '$role->codrole'";
        return $this->executeUpdate($sql);
    }
    ...
    public function queryByDescripcion($value) {
        $this->set($value);
        $sql = "SELECT * FROM db_toquela.role WHERE descripcion  = '$value'";
        return $this->getList($sql);    
    }
    
    public function queryByPrivilegios($value) {
        $this->set($value);
        $sql = "SELECT * FROM db_toquela.role WHERE privilegios  = '$value'";
        return $this->getList($sql);    
    }
    ...
    public function deleteByDescripcion($value) {
        $this->set($value);
        $sql = "DELETE FROM db_toquela.role WHERE descripcion  = '$value'";
        return $this->executeUpdate($sql);
    }
    ...
    public function deleteByPrivilegios($value) {
        $this->set($value);
        $sql = "DELETE FROM db_toquela.role WHERE privilegios  = '$value'";
        return $this->executeUpdate($sql);
    }
    ...
    protected function readRow($row) {
        ...
        $role->privilegios = $row['privilegios'];
        return $role;
    }    
