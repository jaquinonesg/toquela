 ...
 */
class SubComplexMySqlDAO extends ModelDAO implements SubComplexDAO {
    ...
    public function update($subcomplex) {
        ...
        $sql = "UPDATE db_toquela.sub_complex  SET 
		 name = if( strcmp('$subcomplex->name'  , '' ) = 1  , '$subcomplex->name', name ),
		 cod_complex = if( strcmp('$subcomplex->codcomplex'  , '' ) = 1  , '$subcomplex->codcomplex', cod_complex ) WHERE  cod_sub_complex =  '$subcomplex->codsubcomplex'";
        return $this->executeUpdate($sql);
    }
