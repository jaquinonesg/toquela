...
class ScheduleMySqlExtDAO extends ScheduleMySqlDAO {
    ...
    public function isScheduleBetweenHours($day, $start, $end, $cod_sub) {
        $sql = "SELECT 
                  ...
                  WHERE s.day = '$day' 
                    AND (
                      (
                        '$start' BETWEEN s.start_hour 
                        AND s.end_hour
                      ) 
                      OR (
                        '$end' BETWEEN s.start_hour 
                        AND s.end_hour
                      )
                    ) 
    ...
    public function getScheduleByDay($date, $day, $cancha) {
        $sql = "SELECT 
                    ...
                    FROM
                      db_toquela.reserve 
                    WHERE CONVERT(START USING utf8 ) LIKE '$date%' 
                      AND s.cod_schedule = cod_schedule) AS 'status',
                    (SELECT 
                      cod_reserve 
                    FROM
                      db_toquela.reserve 
                    WHERE CONVERT(START USING utf8) LIKE '$date%' 
                      AND s.cod_schedule = cod_schedule) AS cod_reserve 
                  FROM
                    db_toquela.schedule s 
