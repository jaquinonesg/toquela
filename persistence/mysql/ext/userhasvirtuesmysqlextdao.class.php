 ...
 */
class UserHasVirtuesMySqlExtDAO extends UserHasVirtuesMySqlDAO {
    ...
    public function getFavoritePostionGame($cod_user) {
        $sql = "SELECT * FROM db_toquela.virtues v, db_toquela.user_has_virtues uhv WHERE v.cod_virtues = uhv.cod_virtues AND uhv.cod_user = '$cod_user' AND v.cod_virtues_group = 2 AND uhv.`main`= 1 LIMIT 1;";
        return $this->getRow($sql, true);
    }

    public function getSkilledLeg($cod_user) {
        $sql = "SELECT * FROM db_toquela.virtues v, db_toquela.user_has_virtues uhv WHERE v.cod_virtues = uhv.cod_virtues AND uhv.cod_user = '$cod_user' AND v.cod_virtues_group = 1 LIMIT 1";
        return $this->getRow($sql, true);
    }

    public function getLevelGame($cod_user) {
        $sql = "SELECT * FROM db_toquela.virtues v, db_toquela.user_has_virtues uhv WHERE v.cod_virtues = uhv.cod_virtues AND uhv.cod_user = '$cod_user' AND v.cod_virtues_group = 4 LIMIT 1;";
        return $this->getRow($sql, true);
    }
