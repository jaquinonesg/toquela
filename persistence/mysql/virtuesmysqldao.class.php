 ...
 */
class VirtuesMySqlDAO extends ModelDAO implements VirtuesDAO{
...
$this->set($virtues->codvirtuesgroup);
        ...
        $sql = "UPDATE db_toquela.virtues  SET 
		 name = if( strcmp('$virtues->name'  , '' ) = 1  , '$virtues->name', name ),
		 type = if( strcmp('$virtues->type'  , '' ) = 1  , '$virtues->type', type ),
		 cod_virtues_group = if( strcmp('$virtues->codvirtuesgroup'  , '' ) = 1  , '$virtues->codvirtuesgroup', cod_virtues_group ) WHERE  cod_virtues =  '$virtues->codvirtues'";
        return $this->executeUpdate($sql);
    }
