 ...
 */
class VirtuesGroupSqlServerDAO extends ModelDAO implements VirtuesGroupDAO{
...
$this->set($virtuesgroup->description);
        ...
        $sql = "UPDATE db_toquela.virtues_group  SET 
		 name = iif( len('$virtuesgroup->name' ) <> 0  , '$virtuesgroup->name', name ),
		 description = iif( len('$virtuesgroup->description' ) <> 0  , '$virtuesgroup->description', description ) WHERE  cod_virtues_group =  '$virtuesgroup->codvirtuesgroup'";
        return $this->executeUpdate($sql);
    }
