 ...
 */
class DisponibilityComplexSqlServerDAO extends ModelDAO implements DisponibilityComplexDAO{
...
$this->set($disponibilitycomplex->codtariff);
        ...
        $sql = "UPDATE db_toquela.disponibility_complex  SET 
		 day = iif( len('$disponibilitycomplex->day' ) <> 0  , '$disponibilitycomplex->day', day ),
		 hour = iif( len('$disponibilitycomplex->hour' ) <> 0  , '$disponibilitycomplex->hour', hour ),
		 cod_complex = iif( len('$disponibilitycomplex->codcomplex' ) <> 0  , '$disponibilitycomplex->codcomplex', cod_complex ),
		 cod_tariff = iif( len('$disponibilitycomplex->codtariff' ) <> 0  , '$disponibilitycomplex->codtariff', cod_tariff ) WHERE  cod_disponibility_complex =  '$disponibilitycomplex->coddisponibilitycomplex'";
        return $this->executeUpdate($sql);
    }
