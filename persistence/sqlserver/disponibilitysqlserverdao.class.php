 ...
 */
class DisponibilitySqlServerDAO extends ModelDAO implements DisponibilityDAO{
...
$this->set($disponibility->coduser);
        ...
        $sql = "UPDATE db_toquela.disponibility  SET 
		 day = iif( len('$disponibility->day' ) <> 0  , '$disponibility->day', day ),
		 start_hour = iif( len('$disponibility->starthour' ) <> 0  , '$disponibility->starthour', start_hour ),
		 end_hour = iif( len('$disponibility->endhour' ) <> 0  , '$disponibility->endhour', end_hour ),
		 cod_user = iif( len('$disponibility->coduser' ) <> 0  , '$disponibility->coduser', cod_user ) WHERE  cod_disponibility =  '$disponibility->coddisponibility'";
        return $this->executeUpdate($sql);
    }
