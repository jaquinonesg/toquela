 ...
 */
class LocalitySqlServerDAO extends ModelDAO implements LocalityDAO{
...
$this->set($codlocality);
    ...
    public function queryAll($limit = null, $page = null) {
        ...
        $sql = "SELECT TOP 20 * FROM db_toquela.locality $extra";
...
$this->set($locality->main);
        ...
        $sql = "UPDATE db_toquela.locality  SET 
		 name = iif( len('$locality->name' ) <> 0  , '$locality->name', name ),
		 cod_city = iif( len('$locality->codcity' ) <> 0  , '$locality->codcity', cod_city ),
		 main = iif( len('$locality->main' ) <> 0  , '$locality->main', main ) WHERE  cod_locality =  '$locality->codlocality'";
        return $this->executeUpdate($sql);
    }
    ...
    public function clean(){
        ...
        $this->set($value);
        $sql = "SELECT * FROM db_toquela.locality WHERE cod_city  = '$value' ORDER BY name";
        return $this->getList($sql);    
    }
