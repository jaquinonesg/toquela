 ...
 */
class UserHasTeamSqlServerDAO extends ModelDAO implements UserHasTeamDAO{
...
$this->set($codteam);
    ...
    public function update($userhasteam){
        ...
        $sql = "UPDATE db_toquela.user_has_team  SET 
		 status = iif( len('$userhasteam->status' ) <> 0  , '$userhasteam->status', status ) WHERE  cod_user =  '$userhasteam->coduser' AND  cod_team =  '$userhasteam->codteam'";
        return $this->executeUpdate($sql);
    }
