 ...
 */
class TransactionSqlServerDAO extends ModelDAO implements TransactionDAO{
...
$this->set($transaction->coduser);
        ...
        $sql = "UPDATE db_toquela.transaction  SET 
		 amount = iif( len('$transaction->amount' ) <> 0  , '$transaction->amount', amount ),
		 cod_user = iif( len('$transaction->coduser' ) <> 0  , '$transaction->coduser', cod_user ) WHERE  cod_transactions =  '$transaction->codtransactions'";
        return $this->executeUpdate($sql);
    }
