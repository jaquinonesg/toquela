 ...
 */
class CommentSqlServerDAO extends ModelDAO implements CommentDAO{
...
$this->set($comment->coduser);
        ...
        $sql = "UPDATE db_toquela.comment  SET 
		 message = iif( len('$comment->message' ) <> 0  , '$comment->message', message ),
		 cod_match = iif( len('$comment->codmatch' ) <> 0  , '$comment->codmatch', cod_match ),
		 cod_user = iif( len('$comment->coduser' ) <> 0  , '$comment->coduser', cod_user ) WHERE  cod_comment =  '$comment->codcomment'";
        return $this->executeUpdate($sql);
    }
