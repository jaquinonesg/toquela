 ...
 */
class LanguageSqlServerDAO extends ModelDAO implements LanguageDAO{
...
$this->set($language->dialect);
        ...
        $sql = "UPDATE db_toquela.language  SET 
		 word = iif( len('$language->word' ) <> 0  , '$language->word', word ),
		 traduction = iif( len('$language->traduction' ) <> 0  , '$language->traduction', traduction ),
		 dialect = iif( len('$language->dialect' ) <> 0  , '$language->dialect', dialect ) WHERE  cod_language =  '$language->codlanguage'";
        return $this->executeUpdate($sql);
    }
