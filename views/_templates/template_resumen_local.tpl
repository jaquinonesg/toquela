{foreach from=$statistics_local name=foo item=statis key=count}
    {$auxcont = ($init + $count)}
    {if !$solo_format}
        <span id="_resu_local_contend_{$auxcont}">
        {/if}
        {$esconder_statistic = ''}
        {if ($match_info->scorelocal >= 0) && $statis->codtypestatistic == 19}
        ...
        <div class="formato_statistic {$esconder_statistic} btn col-xs-12 col-sm-12 col-ms-12 col-lg-12" data-toggle="popover" title="" data-placement="top" data-content="<p><strong>Minuto: </strong>{$statis->minute}</p><p><strong>Jugador: </strong>{$statis->player->name} {$statis->player->lastname}</p><p><strong>Descripción: </strong>{$statis->description}</p>" role="button" data-original-title="{$statis->type->name}"
             id="_resu_local_{$auxcont}" index="{$auxcont}" minute="{$statis->minute}" date="{$statis->date}" islocal="{$statis->islocal}" typestatistic="{$statis->codtypestatistic}" player="{$statis->coduser}" partido="{$statis->codmatch}" description="{$statis->description}">
            ...
            <div class="col-xs-12 col-sm-12 col-md-6 col-xs-6" style="padding-right: 0">
                ...
                <span class="actions_resumen">
                    ...
                    <button type="button" data-toggle="popover" data-container="body" data-toggle="popover" data-placement="left" data-content="{if !isset($statis->estado) || $statis->estado == 0}No ha sido pagada{else}Pagada{/if}" class="btn_pagar" islocal="{$statis->islocal}" index="{$auxcont}" statistic="{$statis->codstatistics}" typeEstado="{if !isset($statis->estado) || $statis->estado == 0}nopagado{else}pagado{/if}" typestatistic="{$statis->codtypestatistic}">
