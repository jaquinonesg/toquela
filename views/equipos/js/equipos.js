 ...
 * @type {PaginarEquipos}
 */
var PaginarEquipos = (function() {
    var _self = null;
    var timer_buscador = null;
    function PaginarEquipos(clase_alias_paginador, id_contend_load_data, id_form_buscador, url_load_data) {
        _self = this;
        this.process = false;
    ...
    PaginarEquipos.prototype.readConfigPaginator = function() {
    ...
    PaginarEquipos.prototype.loadPaginator = function() {
        if (this.process && this.isajax) {
            this.events();
        }
    };

    PaginarEquipos.prototype.events = function() {
    ...
    PaginarEquipos.prototype.getEquipos = function(genero, tipo, strequipo, pag) {
        if (validate.isEmpty(genero) && validate.isEmpty(tipo) && validate.isEmpty(strequipo)) {
            ...
            if (validate.isNumeric(pag)) {
                $.ajax({
                    type: 'POST',
                    url: base_url + "/equipos/search_equipos",
    ...
    PaginarEquipos.prototype.focusTimeBuscador = function(deprimera) {
    ...
    PaginarEquipos.prototype.loadPaginaInicial = function(pag) {
        if (validate.isNumeric(pag)) {
            $.ajax({
                type: 'POST',
    ...
    PaginarEquipos.prototype.getHtmlLoader = function() {
