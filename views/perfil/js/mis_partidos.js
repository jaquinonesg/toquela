        ...
        if (validate.isEmpty(genero) && validate.isEmpty(tipo) && validate.isEmpty(strequipo)) {
            ...
            if (validate.isNumeric(pag)) {
                $.ajax({
                    ...
                    data: {'genero': genero, 'tipo': tipo, 'strequipo': strequipo, 'pag': pag, 'typeGame': 1},
                    beforeSend: function() {
...
var PaginatorGamesPublic = (function() {
    ...
    PaginatorGamesPublic.prototype.getEquipos = function(genero, tipo, strequipo, pag) {
        if (validate.isEmpty(genero) && validate.isEmpty(tipo) && validate.isEmpty(strequipo)) {
            this.loadPaginaInicial(pag);
        } else {
            if (validate.isNumeric(pag)) {
                $.ajax({
                    ...
                    data: {'genero': genero, 'tipo': tipo, 'strequipo': strequipo, 'pag': pag, 'typeGame': 2},
                    beforeSend: function() {
                        $("#" + _self.id_contend_load_data).html(_self.getHtmlLoader());
                    },
                    success: function(data) {
    ...
    PaginatorGamesPublic.prototype.loadPaginaInicial = function(pag) {
        if (validate.isNumeric(pag)) {
            $.ajax({
                type: 'POST',
                url: _self.url_load_data,
...
$(document).ready(function() {
    var partidos_publicos = $('#partidos_publicos #_contend_equipos').html();
    var partidos_privados = $('#partidos_privados #_contend_equipos').html();
