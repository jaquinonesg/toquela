...
<div class="nuevo_carnet">
    ...
    <div class="col-xs-12 col-sm-12 col-md-9 col-lg-9 init div-acordion-grupos" style="margin-top:10px;">
        <div class="alert alert-info fade in" style="color: #000000;">
            ...
            <p>1) Se recomienda llenar los datos adicionales <a href="{$_params.site}/perfil/index/2" target="_blank">AQUÍ</a>, para poder realizar una vista previa del carnet con sus propios datos por medio del botón "Vista Previa".</p>
            <br>
        ...
        <div class="carnet">
            <div class="actions">
                ...
                <a href="{$_params.site}/carnets/index/preview/{$tournament->codtournament}" target="_blank">
                    <button class="btn btn-default" id="btn_vista_previa">Vista previa</button>
                </a>
                <a href="{$_params.site}/carnets/index/printreport/{$tournament->codtournament}" target="_blank">
                    <button class="btn btn-default" id="btn_reporte">Reporte</button>
                </a>
                <a href="{$_params.site}/carnets/index/export/{$tournament->codtournament}">
            ...
            <div class="contend_carnet col-xs-12 col-sm-9 col-md-9 col-lg-9 well">
                {if isset($carnet->data)}
                    <div class="plantilla_carnet" id="plantilla_0" torneo="{$carnet->codtournament}" nombre="{if isset($carnet->nombre)} {$carnet->nombre} {else} Plantilla {$tournament->name} {/if}" index="0" cod="{$carnet->codtournament}" ancho="8.5" alto="5.4" caras="2" orientacion="h" padding="0.1" fontsize="12" islogo="true" logofilename="{$carnet->logofilename}" logoname="{$carnet->logoname}" cods='{$carnet->cods}' data='{$carnet->data}'>
                    </div>
                {else}
                    <div class="plantilla_carnet" id="plantilla_0" torneo="{$tournament->codtournament}" nombre="Plantilla {$tournament->name}" index="0" cod="" ancho="8.5" alto="5.4" caras="2" orientacion="h" padding="0.1" fontsize="12" islogo="true" logofilename="{$carnet->logofilename}" logoname="{$carnet->logoname}" cods='' data=''>
                    </div>
                {/if}
            </div>
            <div class="clear"><br></div>
...
<span class="display_none">
    <span id="popup_agregar_logo_body">
        <!--div class="contend_agregar_logo">
            ...
            <img src="{$_params.site}/public/img/Logo-Toquela4.png"/>
            <br>
            <br>
            <form id="form_agregar_logo" action="{$_params.site}/carnets/index/loadlogocarnet/" enctype="multipart/form-data" method="post" role="form">
                <input type="hidden" name="torneo" value="{$tournament->codtournament}"/>
                <input type="text" name="namelogo" id="txt_name_logo" class="form-control" value="{$carnet->logoname}" maxlength="40" autofocus placeholder="Nombre logo"/>
                <br>
