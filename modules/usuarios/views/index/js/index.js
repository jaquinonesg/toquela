...
var PaginarJugadores = (function() {
    ...
    PaginarJugadores.prototype.getJugadores = function(genero, posicion, strjugador, pag) {
        if (validate.isNumeric(pag) && (pag > 0)) {
            $.ajax({
                type: 'POST',
                url: base_url + "/usuarios/index/search_jugadores",
...
$(document).ready(function() {
    function borrarPrivilegios() {
        $('.privileges').each(function(i, e) {
            var type = $(e).attr('type');
            var adminval = $(e).val();
            var id = $(e).attr('id');
            var value = adminval.split(' ');
            var valAdmin = value[0];

            if (type === "checkbox") {
                if (valAdmin == "Administrar" || valAdmin == "administrar") {
                    $(e).remove();
                    $('#div_' + id).remove();
                }
            }
    ...
    $('.rol-no-checked').click(function(){
        $('#frm-privileges').fadeOut('slow');
    });
    $('.rol-si-checked').click(function(){
        $('#frm-privileges').fadeIn('slow');
    });
    $('.rol').each(function(i, e) {
        var type = $(e).attr('type');
        if (type === "radio") {
            if (e.checked) {
                var id = $(e).attr('id');
                //cuando el rol es usuario normal, borrar privilegios de administrador
                if (id == 1) {
                    borrarPrivilegios();
                }
            }
    ...
    $('[checked="checked"]').each(function(i, e) {
        var type = $(e).attr('type');
        if (type === "checkbox") {
            e.checked = true;
        }
    ...
    $("#select-reverse").on('click', function() {
        $('.privileges').each(function(i, e) {
            var type = $(e).attr('type');
            var clase = $(e).attr('class');

            if (type === "checkbox") {
                var boolean = $(e).is(':checked');
                e.checked = !boolean;
                if (clase === "privileges dfl-check") {
                    e.checked = true;
                }
            }
        });
    ...
    $("#enviar").on('click', function(e) {
        ...
        // rol 3 por que es ese el codigo del usuario administrador
        if(rol == 3){
            privileges.push(2);
        }
        var dataByComma = privileges.join();
        var id = $('span.id-usuario').attr('id');

        $.ajax({
            url: base_url + '/usuarios/index/modificar',
            type: 'POST',
            data: {'privileges': privileges, 'dataByComma': dataByComma, 'rol': encodeURIComponent(rol), 'id': encodeURIComponent(id)},
            beforeSend: function() {
//                console.log('enviando');
            ...
            success: function(data) {
                console.log(data);
                if (data.status === true) {
                    component.messageSimple('Información actualizada', data.message);
                    $('.modal-default').click(function() {
                        location.reload(true);
                        $("#select-reverse").click();
                    });
                }
                if (data.status === false) {
                    component.messageSimple('Error', data.message, 'warning');
                }
            },
            error: function() {
                console.log('error');
