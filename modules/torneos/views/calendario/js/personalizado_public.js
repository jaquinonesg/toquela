...
var PersonalizadoPublic = function() {
    ...
    this.events = function() {
        ...
        $(document).on('click', '.btn_tabla_reclasificacion', function() {
            base.getDataPosicionesTotalesFase2($(this).attr("torneo"), $(this).attr("fase"));
        });
...
this.getDataPosicionesTotalesFase2 = function(torneo, fase) {
    if (validate.isNumeric(torneo)) {
        $.ajax({
            type: 'POST',
            url: base_url + '/torneos/clasificacion/tablaPosicionesTotal2/' + torneo +'/'+ fase,
            data: {
                "grupo": fase
            },
            success: function(data) {
                if (data.hasOwnProperty('status') && data.hasOwnProperty('html')) {
                    if (data.status) {
                        $("#modal-total-posiciones .modal-body").html(data.html);
                        $("#modal-total-posiciones .modal-title").html('Tabla de reclasificación general de la fase '+ fase);
                        $('#modal-total-posiciones').modal('show');
                    } else {
                        component.popupHtmlHide();
                        component.messageSimple("Calendario", data.message, "danger");
